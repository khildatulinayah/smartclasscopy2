...
<head>
    <meta charset="utf-8">
    <title>Laporan Absensi - {{ $monthName }}</title>
    <center>
    <style>
...
@php $logoPath = public_path('images/logo2.png'); @endphp
        ...
        <div class="holiday-info">
            ...
            @foreach($holidays as $date => $note)
                <div>• {{ \Carbon\Carbon::parse($date)->locale('id')->format('d F Y') }}: {{ $note }}</div>
            @endforeach
        </div>
        @endif
            ...
            <table class="attendance-table">
                <thead>
                    <tr>
                        ...
                        <th colspan="{{ $daysInMonth ?? \Carbon\Carbon::create($year, $month)->daysInMonth }}">Kehadiran</th>
                ...
                <tbody>
                    @foreach($students as $index => $student)
                        ...
                        <tr>
                            ...
                            @for ($day = 1; $day <= $daysInMonth; $day++)
                                ...
                                @endphp
                                <td class="day-col {{ $statusClass }}">{{ $status }}</td>
                            @endfor
                            
                            <td class="total-section">{{ $studentStats['sakit'] }}</td>
                            <td class="total-section">{{ $studentStats['izin'] }}</td>
                            <td class="total-section">{{ $studentStats['alpha'] }}</td>
                        </tr>
                    @endforeach
                    ...
                    <tr class="total-section">
                        ...
                        @for ($day = 1; $day <= $daysInMonth; $day++)
                            ...
                            @endphp
                        @endfor
                        <td>{{ $stats['totalSakit'] }}</td>
                        <td>{{ $stats['totalIzin'] }}</td>
                        <td>{{ $stats['totalAlpha'] }}</td>
                    </tr>
                </tbody>
            </table>
        @else
