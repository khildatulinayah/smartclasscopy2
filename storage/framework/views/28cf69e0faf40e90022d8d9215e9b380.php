...
<div class="dashboard-layout">
    ...
    <?php if(!empty($pendingAdjustmentCount) && $pendingAdjustmentCount > 0): ?>
        <div class="bg-indigo-50 border border-indigo-200 rounded-xl p-4 mb-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                ...
                <div class="text-right">
                    <button type="button" onclick="openPaymentAdjustmentModal()"
                        class="inline-flex items-center px-3 py-2 rounded-full bg-indigo-100 text-indigo-800 text-sm font-semibold hover:bg-indigo-200 transition-colors">
                        Lihat Adjustment
                    </button>
                </div>
            </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-4 mb-8">
        ...
        <?php if(!empty($adjustmentStudentCount) && $adjustmentStudentCount > 0): ?>
        <button type="button" onclick="openPaymentAdjustmentModal()"
            class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 text-left hover:shadow-md hover:border-indigo-300 transition-all cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-400"
            title="Klik untuk lihat detail penyesuaian">
            <div class="flex items-center mb-2">
                ...
                <div>
                    <div class="text-2xl font-bold text-indigo-600"><?php echo e($adjustmentStudentCount); ?></div>
                    <div class="text-sm text-gray-500">Penyesuaian</div>
                </div>
            </div>
        </button>
        <?php endif; ?>
...
function openPaymentAdjustmentModal() {
    const modal = document.getElementById('paymentAdjustmentModal');
...
document.getElementById('paymentAdjustmentModal')?.addEventListener('click', function(e) {
    if (e.target === this) {
