...
function showDifferenceModal(paymentId, difference, studentName, week) {
    ...
    document.getElementById('diff_old_amount').textContent = `Rp {{ number_format($weeklyPaymentAmount - $weeklyPaymentAmount, 0, ',', '.') }}`;
    document.getElementById('diff_new_amount').textContent = `Rp {{ number_format($weeklyPaymentAmount, 0, ',', '.') }}`;
